...
- **The recovery key is printed once and must be acknowledged.** Outside Entra ID it exists nowhere but on the volume and on the paper you write it on. A password is asked for in virtual hard disk mode only. **The script does not judge it - BitLocker does.** Whatever you type is handed straight to Windows, and if Windows refuses it you are told what it said and asked for another. Measured on an unmanaged machine, that means at least 8 and at most 256 characters and nothing else: no character classes, and non-ASCII accepted. Group policy can require more, and a domain controller can require the password to meet the domain's complexity rules
